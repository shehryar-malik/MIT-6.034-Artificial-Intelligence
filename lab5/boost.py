...
class BoostClassifier(Classifier):
    ...
    def classify(self, obj):
        ...
        for classifier in self.classifiers:
            result += classifier[1] * classifier[0].classify(obj)
        if result >= 0:
            return 1
        return -1
    ...
    def update_weights(self, best_error, best_classifier):
        ...
        new_data_weights = []
        for datum, dweight in zip(self.data, self.data_weights):
            if self.standard.classify(datum) == best_classifier.classify(datum):
                value = dweight *(1/float(2)) * (1/float(1-best_error))
                new_data_weights += [value]
            else:
                value = dweight * (1/float(2)) * (1/float(best_error))
                new_data_weights += [value]
